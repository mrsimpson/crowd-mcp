...
describe("AgentRegistry", () => {
  ...
  describe("syncFromDocker", () => {
    ...
    it("should call listContainers with all: false to exclude exited containers", async () => {
      (mockDocker.listContainers as ReturnType<typeof vi.fn>).mockResolvedValue(
        [],
      );
    ...
    it("should emit agent:removed event when removing agents that are no longer running", async () => {
      // First sync: add agent
      const initialContainers = [
        {
          Id: "container-abc",
          Names: ["/agent-123"],
