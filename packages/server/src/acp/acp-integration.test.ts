...
vi.mock("./acp-container-client.js", () => ({
  ACPContainerClient: vi
    .fn()
    .mockImplementation((agentId: string, containerId: string) => ({
      agentId,
      containerId,
...
describe("ACP Integration", () => {
  ...
  describe("ACPClientManager", () => {
    ...
    it("should forward messages to healthy clients", async () => {
      ...
      // Forward message
      await expect(
        clientManager.forwardMessage(agentId, message),
    ...
    it("should throw error when forwarding to non-existent client", async () => {
      ...
      await expect(
        clientManager.forwardMessage("non-existent", message),
      ).rejects.toThrow();
    });
  });
  ...
  describe("ACPMessageForwarder", () => {
    it("should forward messages to agent recipients", async () => {
      ...
      // Forward message
      await expect(
        messageForwarder.forwardMessage(message),
      ).resolves.not.toThrow();
    });
