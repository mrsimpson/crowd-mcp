...
export class ACPContainerClient {
  ...
  private eventEmitter?: EventEmitter; // Event emitter for streaming updates
  ...
  private async performHandshake(): Promise<void> {
    ...
    console.log(
      `📋 Created session with ${this.mcpServers.length} MCP servers for ${this.agentId}`,
    );
    ...
    if (!this.sessionId) {
      throw new Error(
        `Failed to create session for agent ${this.agentId} - no session ID received`,
      );
    }

    console.log(
      `✅ Session established for ${this.agentId}: ${this.sessionId}`,
    );
  }
  ...
  private processMessage(line: string): void {
    try {
      ...
      if (
        ...
        message.id === this.sessionNewRequestId
      ) {
        this.sessionId = message.result.sessionId;
        console.log(
          `✅ Session ID captured for ${this.agentId}: ${this.sessionId}`,
        );
      }
      ...
      if (
        ...
        if (this.eventEmitter) {
          this.eventEmitter.emit("agent:streaming:chunk", {
            agentId: this.agentId,
            chunk: content,
            accumulated: this.currentResponse,
          });
        }
      }
      ...
      if (message.result?.stopReason === "end_turn") {
        console.log(
          `✅ [${this.agentId}] Agent completed response: "${this.currentResponse}"`,
        );

        // Emit streaming completion event
        if (this.eventEmitter) {
          this.eventEmitter.emit("agent:streaming:complete", {
            agentId: this.agentId,
        ...
        if (this.currentResponse.trim() && this.messageRouter) {
          ...
          this.messageRouter
            ...
            .catch((error: unknown) => {
              console.error(
                `❌ [${this.agentId}] Failed to send response to message router:`,
                error,
              );
            });
        }
  ...
  private async waitForSessionId(): Promise<void> {
    ...
    if (this.logger) {
      await this.logger.debug(
        "Starting exponential backoff polling for session ID",
        {
          maxAttempts,
          initialDelay: delay,
        },
    ...
    while (attempt < maxAttempts && !this.sessionId) {
      ...
      if (!this.sessionId) {
        ...
        console.log(
          `⏳ Waiting for session ID (attempt ${attempt}/${maxAttempts}, next delay: ${delay}ms)`,
        );
  ...
  async sendPrompt(message: {
    ...
    try {
      ...
      if (this.eventEmitter) {
        this.eventEmitter.emit("agent:streaming:start", {
          agentId: this.agentId,
